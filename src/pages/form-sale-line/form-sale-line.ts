...
@IonicPage()
@Component({
  selector: 'page-form-sale-line',
  templateUrl: 'form-sale-line.html',
})
export class FormSaleLinePage {
  ...
  changeValue(event,type){
    ...
    switch(type){
      case 'unity':{ 
        this.line.product_uom.name = this.find(event, this.listUnits, '');
        break;
      }      
    }
  }
    ...
    popover.onDidDismiss((result)=>{
        ...
        if(result){ 
          ...
          for(let i = 0; i< result.tab.length; i++){
            	if(this.line.tax_id.indexOf(result.tab[i])==-1 || this.line.tax_id.length==0)
              		this.line.tax_id.push(result.tab[i]);
          }
